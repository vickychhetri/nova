...
func main() {
	...
	topMenuBar := widgets.MenuBar(
		widgets.NewMenu("File",
			widgets.ShortcutItem("Start Scan", "Ctrl+S", func() {
			...
			widgets.ShortcutItem("Export Discovered Hosts", "Ctrl+E", func() {
				statusText.Set(fmt.Sprintf("Exported %d hosts to report.json", len(hostsDiscovered.Get())))
			}),
			widgets.DividerItem(),
		...
		widgets.NewMenu("Edit",
			widgets.ShortcutItem("Clear Results", "Ctrl+L", func() {
				hostsDiscovered.Set([]HostRecord{})
				openPortsTotal.Set(0)
				statusText.Set("Cleared all scan results.")
			}),
		),
		widgets.NewMenu("Scan",
			widgets.ActionItem("Quick Ping Sweep (/24)", func() {
				startScan("192.168.1.0/24", "80,443", 500*time.Millisecond, 50)
			}),
			widgets.ActionItem("Full Port Scan (Top 1000)", func() {
				startScan("192.168.1.1", "21,22,25,53,80,110,143,443,3306,5432,8080", 500*time.Millisecond, 20)
			}),
		),
		widgets.NewMenu("Tools",
			widgets.ActionItem("Lookup Subnet Gateway", func() {
				statusText.Set("Gateway: 192.168.1.1 (UP)")
			}),
		),
		widgets.NewMenu("Help",
			widgets.ActionItem("About Nova IP Scanner", func() {
				statusText.Set("Nova Concurrent Multi-threaded IP Scanner v2.0.")
			}),
		),
	)
